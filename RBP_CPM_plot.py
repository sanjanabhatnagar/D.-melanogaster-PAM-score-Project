...
modencode_expr_Mike = pd.read_csv('logCPM_means.csv') # ModEncode data processed by Michael Zoberman

modencode_expr_Mike_RBP = modencode_expr_Mike[['Sample.Name','FBgn0261552']] # Selecting an RBP 
...
modencode_expr_Mike_RBP_tissue.to_csv('Pasilla_tissue_expression_logCPM.csv')
modencode_expr_Mike_RBP_dev.to_csv('Pasilla_dev_expression_logCPM.csv')

# Same code to process dev stage data!
modencode_expr_Mike_RBP_tissue.drop(columns='Unnamed: 0', inplace=True)
modencode_expr_Mike_RBP_tissue.rename(columns={'FBgn0261552':'Pasilla'}, inplace=True) 

modencode_expr_Mike_RBP_tissue_CPM = modencode_expr_Mike_RBP_tissue.copy()
modencode_expr_Mike_RBP_tissue_CPM.iloc[:,1:] = pow(2, modencode_expr_Mike_RBP_tissue.iloc[:,1:]) # Since expression is reported in log base 2 CPM values.

# Plotting expression CPM values across tissues and developmental stages.
import matplotlib.pyplot as plt
import seaborn as sns
plt.figure(figsize=(15,6))
for i in modencode_expr_Mike_RBP_tissue_CPM.iloc[:,1:].columns:
    sns.lineplot(data =modencode_expr_Mike_RBP_tissue_CPM,  x='Sample.Name', y=i, label=i)
plt.xticks(rotation=45, ha='right')
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
plt.ylabel('CPM', fontsize=14)
plt.tight_layout()
plt.grid(False)
